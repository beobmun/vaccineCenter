...
class VaccineCenterSearchVC: UIViewController {
    ...
    @IBAction func inquiryBtnClicked(_ sender: Any) {
        if sidoTextField.text == ""{
            ...
            self.performSegue(withIdentifier: "VaccineTableVC", sender: self)
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let id = segue.identifier, "VaccineTableVC" == id {
            if let controller = segue.destination as? VaccineTableVC {
                if self.sidoTextField.text == "세종특별자치시" {
                    controller.sido = self.sidoTextField.text
                    controller.sigungu = ""
                } else {
                    controller.sido = self.sidoTextField.text
                    controller.sigungu = self.sigunguTextField.text
                }
            }
        }
    }
...
extension VaccineCenterSearchVC: UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
